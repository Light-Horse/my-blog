  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans pb-10">
      ...
      <div className="max-w-4xl mx-auto p-2 md:p-4">
              ...
              <div className="overflow-x-auto relative">
                <table className={`w-full text-left whitespace-nowrap ${TABLE_STYLES.fontSize}`}>
                  <thead className={`${TABLE_STYLES.headerFontSize} text-gray-400 uppercase bg-gray-800`}>
                    <tr>
                      <th onClick={() => handleSort('name')} className={`sticky left-0 z-20 bg-gray-800 ${TABLE_STYLES.cellPadding} ${TABLE_STYLES.rowPadding} border-r border-gray-700 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)] cursor-pointer hover:bg-gray-700`}>
                        <div className="flex items-center">종목 {getSortIcon('name')}</div>
                      </th>
                      <th onClick={() => handleSort('roiLump')} className={`${TABLE_STYLES.cellPadding} ${TABLE_STYLES.rowPadding} text-right cursor-pointer hover:bg-gray-700`}>
                        <div className="flex items-center justify-end">거치 {getSortIcon('roiLump')}</div>
                      </th>
                      <th onClick={() => handleSort('roiDca')} className={`${TABLE_STYLES.cellPadding} ${TABLE_STYLES.rowPadding} text-right cursor-pointer hover:bg-gray-700`}>
                          <div className="flex items-center justify-end">적립 {getSortIcon('roiDca')}</div>
                      </th>
                      <th onClick={() => handleSort('roiComp')} className={`${TABLE_STYLES.cellPadding} ${TABLE_STYLES.rowPadding} text-right cursor-pointer hover:bg-gray-700`}>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    ...
                    {loading ? (
                        ...
                        <tr key={row.id} className="bg-gray-900 hover:bg-gray-800 transition">
                            {/* 종목명 & 티커 */}
                            <td
                            className={`sticky left-0 z-10 bg-gray-900 ${TABLE_STYLES.cellPadding} ${TABLE_STYLES.rowPadding} border-r border-gray-800 font-bold text-gray-100 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)] max-w-[100px] md:max-w-none truncate`}
                            >
                            {row.name}
                            ...
                            <>
                                <td
                                className={`${TABLE_STYLES.cellPadding} ${TABLE_STYLES.rowPadding} text-right font-bold transition-colors duration-300`}
                            ...
                            {/* 추천일 & 추천인 */}
                            <td
                            className={`${TABLE_STYLES.cellPadding} ${TABLE_STYLES.rowPadding} text-center font-mono text-[10px] text-gray-500 tracking-tighter`}
                            >
        ...
        {/* --- [2] 종목 추가 --- */}
        {activeTab === "종목 추가" && (
            <StockAddForm onSuccess={() => { setActiveTab("대시보드"); fetchData(20); }} />
        )}
